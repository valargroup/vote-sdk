...
fn wait_for_tree_size(round_id: &str, target: u64, timeout: Duration) {
    ...
    let actual = commitment_tree_next_index(round_id).unwrap_or(0);
    panic!(
...
fn voter_throughput_stress() {
    ...
    wait_for_tree_size(&round_id_hex, expected_after_deleg, timeout);
    ...
    let (on_chain_height, on_chain_root, tree_size) =
        commitment_tree_latest(&round_id_hex).expect("query tree latest");
    eprintln!(
    ...
    wait_for_tree_size(&round_id_hex, expected_after_cast, timeout);
    ...
    let final_tree_size = commitment_tree_next_index(&round_id_hex).unwrap_or(0);
