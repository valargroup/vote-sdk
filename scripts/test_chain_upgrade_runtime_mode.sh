...
grep -q "^ExecStart=${COSMOVISOR_BIN_TEST} run start --home ${MIGRATE_HOME} --serve-ui --ui-dist /opt/ui/dist$" "$DROPIN_PATH" || fail "drop-in ExecStart did not use resolved cosmovisor binary"
