...
export function EndorsersPage({ wallet }: EndorsersPageProps) {
  ...
  const refreshEndorsedRounds = useCallback(async (endorserID: string) => {
    ...
    setEndorsedRoundIDs(new Set(resp.vote_round_ids.map(base64ToHex)));
  }, []);
  ...
  const refresh = useCallback(async () => {
    ...
    await refreshEndorsedRounds(nextSelected);
  }, [refreshEndorsedRounds, selectedEndorserID, wallet.address]);
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <section className="bg-surface-1 border border-border rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border-subtle">
            <h2 className="text-sm font-semibold text-text-primary">Configured endorsers</h2>
          </div>
          {endorsers.length === 0 ? (
            <p className="p-4 text-xs text-text-muted">No endorsers configured yet.</p>
          ) : (
            <div className="divide-y divide-border-subtle">
              {endorsers.map((endorser) => (
                <div key={endorser.endorser_id} className="p-4 flex items-center justify-between gap-3">
