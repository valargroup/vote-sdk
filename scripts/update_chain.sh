...
readonly UPDATE_COMMON_SHA256='ef6d22408faebae851d4ab4d44ec4071ea49fcf2c9ac3127af11cc32bf164edb'
...
INSTALL_CLI_SET=0
TIMEOUT_SECS=120
WRAPPER_SVOTED_START_ARGS="${SVOTE_WRAPPER_SVOTED_START_ARGS:-}"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --timeout-secs=*)
      ...
      shift
      ;;
    --wrapper-svoted-start-args)
      [ "$#" -ge 2 ] || svote_upgrade_die "--wrapper-svoted-start-args requires a value."
      WRAPPER_SVOTED_START_ARGS="$2"
      shift 2
      ;;
    --wrapper-svoted-start-args=*)
      WRAPPER_SVOTED_START_ARGS="${1#--wrapper-svoted-start-args=}"
      shift
      ;;
    --repo)
...
run_stage_first() {
  local staged_svoted upgrade_bin resolved_dir candidate_bin
  ...
  candidate_bin="${TMP_DIR}/prepared/bin/svoted"
  svote_upgrade_stage_binary "$extracted_svoted" "$candidate_bin"
  svote_upgrade_write_artifact_identity "$PLAN_NAME" "$RELEASE_TAG" "$candidate_bin"
  svote_upgrade_install_cosmovisor "$TMP_DIR"
  [ -x "$COSMOVISOR_BIN" ] || svote_upgrade_die "Missing cosmovisor binary: ${COSMOVISOR_BIN}"

  # Downloads may span a plan replacement, cancellation, or the halt height.
  # Recheck before touching the existing preparation or genesis binary.
  svote_upgrade_validate_scheduled_plan "$PLAN_NAME" "$ALLOW_NO_PLAN"
  svote_upgrade_verify_artifact_identity "$PLAN_NAME" "$RELEASE_TAG" "$candidate_bin"
  ...
  svote_upgrade_commit_prepared_artifact "$candidate_bin" "$upgrade_bin"
