...
| Offline validator | REGISTERING phase times out after `DefaultContributionTimeout` (30 min), the pending round is finalized, and a later create transaction can retry the same vote metadata with a fresh height-derived `vote_round_id` |
