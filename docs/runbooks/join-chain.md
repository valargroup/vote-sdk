...
| `max_concurrent_proofs_v3` | `2` | Parallel proof goroutines (~500 MB each, so ~1 GB at two). |

The current binary ignores the legacy `max_concurrent_proofs` and
`max_concurrent_proofs_v2` keys. The binary switch therefore moves every
validator to the benchmarked two-worker default without a manual `app.toml`
edit. Use the v3 key for an explicit override, and check the host has roughly
1 GB free beyond the validator before using two.
