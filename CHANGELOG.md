...
- Bind dynamic round attestations to the round ID, election-authority key,
  PIR layout, and YPIR `poly_len` with domain-separated `auth_version: 2`
  signatures, preventing cross-round and parameter replay. The admin UI
  intentionally fixes the current `19/12/7` PIR layout and `poly_len` 4096 at
  the authorization point to avoid an additional network dependency. This
  matches the wallet-side verification in
