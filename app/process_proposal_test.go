...
func TestValidateInjectedPartialDecryptEntries(t *testing.T) {
	validPartial := elgamal.PallasGenerator().ToAffineCompressed()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = app.ValidateInjectedPartialDecryptEntries(ta.VoteKeeper(), kvStore, round, msg)
			if tc.wantErr {
