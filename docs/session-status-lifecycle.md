...
- **Trigger (timeout path)**: `EndBlocker` — DEALT phase timeout with at least `required(n) = ceil(4n/5)` acks; non-ackers are stripped from the ceremony snapshot
...
- **Trigger (DEALT timeout failure)**: `EndBlocker` — the ack set does not meet `required(n) = ceil(4n/5)`
