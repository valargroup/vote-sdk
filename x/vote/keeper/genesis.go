...
func (k *Keeper) InitGenesis(kvStore store.KVStore, genesis *types.GenesisState) error {
	...
	for _, rt := range genesis.RoundTrees {
		if rt.TreeState != nil {
			...
			// Genesis carries leaves and roots, but not the Rust shard data.
			// Keep the logical tree state intact and explicitly request one
			// replay from the imported leaves.
			if rt.TreeState.NextIndex > 0 {
				if err := kvStore.Set(types.RoundTreeReplayPendingKey(rt.VoteRoundId), []byte{1}); err != nil {
					return err
				}
			}
		}
