...
- **Ceremony quorum formula:** bounded-subset finalization uses a quorum
  ...
  `t(n) + x(n)` preserves tally liveness after activation. Because integer
  rounding means the two sides are not always equal, the implementation takes
  the stricter side with `max(...)`. This gives the invariant
  `required(n) - f(n) >= t(n)`, so one adversarial pool of size `f(n)` can be
  spent either before activation (non-contribution / non-ack) or after
  activation (partial-decryption withholding) without dropping below the tally
  threshold.
...
18. `x/vote/keeper/keeper_ceremony.go` — `ThresholdForN`, ceremony quorum helpers.
...
- **Check:** the auditor must (i) recompute `t`, `f`, and `required` for every
  ...
  than a half-ack quorum or `nAcks >= t`, and (iv) confirm
...
2. Enumerate t, f, and required(n) for n in [2..16]. Verify timeout
   ...
   required(n), not only t or a half-ack quorum.
