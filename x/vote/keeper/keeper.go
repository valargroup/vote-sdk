...
// SlashingKeeper defines the slashing module interface needed by ceremony
// timeout jailing.
type SlashingKeeper interface {
	Jail(ctx context.Context, consAddr sdk.ConsAddress) error
	JailUntil(ctx context.Context, consAddr sdk.ConsAddress, jailTime time.Time) error
	DowntimeJailDuration(ctx context.Context) (time.Duration, error)
}
...
// SetStakingKeeper replaces the staking keeper. Used by focused tests that
// construct the vote keeper directly.
func (k *Keeper) SetStakingKeeper(sk StakingKeeper) {
	k.stakingKeeper = sk
}

// SetSlashingKeeper replaces the slashing keeper. Used by depinject wiring and
// focused tests that construct the vote keeper directly.
func (k *Keeper) SetSlashingKeeper(sk SlashingKeeper) {
	k.slashingKeeper = sk
}

// SetBankKeeper replaces the bank keeper. Used by focused tests that construct
