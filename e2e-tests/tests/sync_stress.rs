...
fn sync_stress_multi_delegation() {
    ...
    let pre_blast_next_index = commitment_tree_next_index(&round_id_hex).unwrap_or(0);
    ...
    loop {
        if let Some(next) = commitment_tree_next_index(&round_id_hex) {
