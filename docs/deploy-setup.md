...
| `VM_PRIVKEYS`      | Repository  | Comma-separated 64-char hex secp256k1 private keys for the bootstrap vote-manager set (any-of-N). Each derived address becomes a vote manager; the ~1B usvote stake pool is split evenly across the set. |
| `CEREMONY_SSH_KEY` | Environment (`production`) | Ed25519 private key for ceremony bootstrap SSH.       |

Generate each vote manager key with `openssl rand -hex 32` and join them with commas: `VM_PRIVKEYS=<hex1>,<hex2>,<hex3>`. Each derived address is imported as a vote-manager account during chain initialization; any one of them can authorize vote-manager-gated operations (any-of-N). The stake pool is split evenly across vote managers to preserve total chain supply regardless of `N`. **Never commit these keys to the repository** — locally they are loaded from `.env` (see `.env.example`).
