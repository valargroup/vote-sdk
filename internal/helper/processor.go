...
func (p *Processor) processShare(ctx context.Context, share QueuedShare) (*shareBroadcastResult, error) {
	...
	if err != nil {
		return nil, failedShareAttemptError(failureStageDecodeRoundID, fmt.Errorf("decode vote_round_id: %w", err))
	}
	var roundID [32]byte
	if len(roundBytes) != 32 {
		return nil, failedShareAttemptError(failureStageDecodeRoundID, fmt.Errorf("vote_round_id must be 32 bytes, got %d", len(roundBytes)))
	}
	...
	if err != nil {
		return nil, retryableShareError(failureStageTreeStatus, fmt.Errorf("read tree status: %w", err))
	}
	if status.LeafCount == 0 {
		return nil, retryableShareError(failureStageTreeStatus, fmt.Errorf("commitment tree is empty"))
	}
	if share.Payload.TreePosition >= status.LeafCount {
		return nil, retryableShareError(
	...
	if err != nil {
		return nil, retryableShareError(failureStageMerklePath, fmt.Errorf("compute merkle path: %w", err))
	}

	// Decode share_comms.
	var shareComms [16][32]byte
	if len(share.Payload.ShareComms) != 16 {
		return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("expected 16 share_comms, got %d", len(share.Payload.ShareComms)))
	}
	for i, c := range share.Payload.ShareComms {
		cBytes, err := base64.StdEncoding.DecodeString(c)
		if err != nil {
			return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("decode share_comms[%d]: %w", i, err))
		}
		if len(cBytes) != 32 {
			return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("share_comms[%d] must be 32 bytes, got %d", i, len(cBytes)))
		}
	...
	if err != nil {
		return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("decode primary_blind: %w", err))
	}
	if len(pbBytes) != 32 {
		return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("primary_blind must be 32 bytes, got %d", len(pbBytes)))
	}
	...
	if err != nil {
		return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("decode enc_share.c1: %w", err))
	}
	if len(c1Bytes) != 32 {
		return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("enc_share.c1 must be 32 bytes, got %d", len(c1Bytes)))
	}
	c2Bytes, err := base64.StdEncoding.DecodeString(share.Payload.EncShare.C2)
	if err != nil {
		return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("decode enc_share.c2: %w", err))
	}
	if len(c2Bytes) != 32 {
		return nil, failedShareAttemptError(failureStageDecodePayload, fmt.Errorf("enc_share.c2 must be 32 bytes, got %d", len(c2Bytes)))
	}
	...
	span.Finish(err)
	if err != nil {
		return nil, wrapProofGenerateError(err)
	}
	...
	if err != nil {
		return nil, wrapSubmitError(err)
	}
	if result.Code != 0 {
		...
		return nil, failedShareAttemptError(failureStageSubmitChain, fmt.Errorf("chain rejected tx (code %d): %s", result.Code, result.Log))
	}
