...
`t(n) + x(n)` preserves tally liveness after activation. Integer rounding means
the two sides are not always equal, so the implementation takes the stricter
side with `max(...)`.
...
timeout activation should require at least `required(n)` survivors. The
`n - y(n)` side bounds pre-activation missing validators, while `t(n) + x(n)`
preserves tally liveness if validators withhold after activation. Because
integer rounding means the two sides are not always equal, the implementation
takes the stricter side with `max(...)`. For `n=5`, this gives `t=3` and
`required=4`, so one surviving partial-decryption withholder still leaves three
honest partials.
