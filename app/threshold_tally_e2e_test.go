...
// TestThresholdTallyLifecycle is an end-to-end integration test of the
// threshold tally pipeline with per-validator DLEQ proofs. It drives the
// chain from a seeded TALLYING round through FINALIZED using the real
// PrepareProposal → FinalizeBlock → Commit cycle, verifying that:
//   - The proposer's partial decryptions carry valid DLEQ proofs
//   - The SubmitPartialDecryption handler verifies the proofs on-chain
//   - Lagrange combination correctly reconstructs vote totals
//
...
func TestThresholdTallyLifecycle(t *testing.T) {
	...
	// with DLEQ proofs proving log_G(VK_2) == log_{C1}(D_2).
	var v2Entries []*types.PartialDecryptionEntry
	for _, acc := range accums {
		Di2 := acc.ct.C1.Mul(shares[1].Value)
		proof, err := elgamal.GeneratePartialDecryptDLEQ(shares[1].Value, acc.ct.C1)
		require.NoError(t, err)
	...
	// -----------------------------------------------------------------------
	// DLEQ verification: confirm the proposer's stored entries carry valid
	// DLEQ proofs that were generated by PrepareProposal and verified by the
	// SubmitPartialDecryption handler during FinalizeBlock.
	// -----------------------------------------------------------------------

	vk1Point, err := elgamal.UnmarshalPublicKey(vks[0])
	require.NoError(t, err)

	for _, acc := range accums {
		entry, err := ta.VoteKeeper().GetPartialDecryption(
			kvStore, roundID, 1, acc.proposalID, acc.decision)
		require.NoError(t, err)
		require.NotNil(t, entry, "proposer entry missing for (proposal=%d, decision=%d)",
			acc.proposalID, acc.decision)
		require.Len(t, entry.DleqProof, elgamal.DLEQProofSize,
			"proposer DLEQ proof must be %d bytes for (proposal=%d, decision=%d)",
			elgamal.DLEQProofSize, acc.proposalID, acc.decision)

		Di, err := elgamal.UnmarshalPoint(entry.PartialDecrypt)
		require.NoError(t, err)
		err = elgamal.VerifyPartialDecryptDLEQ(entry.DleqProof, vk1Point.Point, acc.ct.C1, Di)
		require.NoError(t, err,
			"proposer DLEQ proof must verify for (proposal=%d, decision=%d)",
			acc.proposalID, acc.decision)
	}
