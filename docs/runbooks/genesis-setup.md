...
4. **`upload-genesis`** — fetches `genesis.json` from `localhost:1317/shielded-vote/v1/genesis` on the primary, uploads it to `s3://shielded-vote/<genesis-key>` (for example `genesis/svote-1/genesis.json` or `genesis/zvote-1/genesis.json`), and clears `s3://shielded-vote/<snapshots-prefix>/` so joiners cannot restore a pre-reset snapshot from the same environment. This is the canonical source joining nodes pull from.
