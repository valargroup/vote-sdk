...
fn voting_flow_librustvoting_path() {
    ...
    // Read the EA keypair from the chain's keypair files (generated by `zallyd ea-keygen`
    // during `make init`). The chain holds the secret key for auto-tally; we need both
    // for ceremony bootstrap (ea_sk) and session creation (ea_pk).
    let ea_pk_path = std::env::var("ZALLY_EA_PK_PATH").unwrap_or_else(|_| {
        let home = std::env::var("HOME").expect("HOME env var must be set");
        format!("{}/.zallyd/ea.pk", home)
    });
    let ea_sk_path = std::env::var("ZALLY_EA_SK_PATH").unwrap_or_else(|_| {
        let home = std::env::var("HOME").expect("HOME env var must be set");
        format!("{}/.zallyd/ea.sk", home)
    });
    ...
    let ea_pk_bytes: [u8; 32] = ea_pk_bytes_vec
        .as_slice()
        .try_into()
        .expect("EA PK must be exactly 32 bytes");
    log_step("Setup", &format!("reading EA secret key from {}", ea_sk_path));
    let ea_sk_bytes_vec = std::fs::read(&ea_sk_path)
        .unwrap_or_else(|e| panic!("failed to read EA SK from {}: {}", ea_sk_path, e));
    let ea_sk_bytes: [u8; 32] = ea_sk_bytes_vec
        .as_slice()
        .try_into()
        .expect("EA SK must be exactly 32 bytes");

    // ---- Step 0: Bootstrap ceremony ----
    ...
    log_step(
        "Step 10",
        &format!("sending 4 share payloads to helper server at {}", helper_url),
    );
