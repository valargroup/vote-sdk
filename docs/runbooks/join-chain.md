...
The artifact base is `https://shielded-vote.nyc3.digitaloceanspaces.com`.
For a custom bucket, fetch `join.sh` from that bucket and pass the same base to
the script:

```bash
curl -fsSL https://shielded-vote.nyc3.digitaloceanspaces.com/join.sh \
  | SVOTE_DO_SPACES_BASE=https://shielded-vote.nyc3.digitaloceanspaces.com bash
```

`SVOTE_DO_SPACES_BUCKET=shielded-vote` is equivalent for buckets in `nyc3`.
...
curl -fsSL https://shielded-vote.nyc3.digitaloceanspaces.com/join.sh | bash -s -- --tls-mode skip

# Install Caddy for a static DNS hostname.
curl -fsSL https://shielded-vote.nyc3.digitaloceanspaces.com/join.sh | bash -s -- --tls-mode custom --domain val.example.org

# Install Caddy with an auto-detected <ip>.sslip.io hostname.
curl -fsSL https://shielded-vote.nyc3.digitaloceanspaces.com/join.sh | bash -s -- --tls-mode auto
```
...
| `max_concurrent_proofs_v2` | `8` | Parallel proof goroutines (~500 MB each, so ~4 GB at eight). |

The v1.4 binary ignores the legacy `max_concurrent_proofs` key. An existing
validator with only the legacy key falls back to one worker after the
coordinated binary switch, without a manual `app.toml` edit — and one worker is
now below the benchmarked value, so set the v2 key explicitly. Check the host
has roughly 4 GB free beyond the validator before using eight.
...
```bash
SVOTE_DO_SPACES_BASE="${SVOTE_DO_SPACES_BASE:-https://shielded-vote.nyc3.digitaloceanspaces.com}"
...
and its snapshot archive before stopping the service. It derives the expected
chain ID from the validator's local `config/genesis.json`; an optional
`SVOTE_CHAIN_ID` override must match that genesis. After the snapshot is staged,
it stops `svoted`, preserves the local
...
`config/node_key.json`, `keyring-test/`, `pallas.*`, or `ea.*`. Active-round
`share.*` and `coeffs.*` ceremony material and the local `helper.db` queue also
remain outside the replaced `data/` directory. Missing snapshot metadata is
fatal for this recovery path; the script does not fall back to genesis for an
already-bonded validator.
...
| `SVOTE_CHAIN_ID` | Local `config/genesis.json` | Optional assertion that must match the existing validator's chain ID. |
...
2. Discover the network and capture the seed peer. The dynamic voting-config payload is published at `voting.valargroup.dev` and defaults to production when no environment path is provided. Source edits live in [token-holder-voting-config](https://github.com/valargroup/token-holder-voting-config/blob/main/prod/dynamic-voting-config.json). Override `VOTING_CONFIG_URL` for staging mirrors.

   ```bash
   VOTING_CONFIG_URL="${VOTING_CONFIG_URL:-https://voting.valargroup.dev/prod/dynamic-voting-config.json}"
...
| `SVOTE_DO_SPACES_BASE` | `https://shielded-vote.nyc3.digitaloceanspaces.com` | Artifact base for `version.txt`, release tarballs, `genesis.json`, and the `svoted-wrapper.sh` fallback. Set this when joining from a custom bucket. |
| `SVOTE_DO_SPACES_BUCKET` | unset | Bucket-name shortcut for `SVOTE_DO_SPACES_BASE`, using `SVOTE_DO_SPACES_REGION` or `nyc3`. |
| `SVOTE_DO_SPACES_REGION` | `nyc3` | Region used only when `SVOTE_DO_SPACES_BUCKET` is set without `SVOTE_DO_SPACES_BASE`. |
...
| `VOTING_CONFIG_URL` | `https://voting.valargroup.dev/prod/dynamic-voting-config.json` | Canonical dynamic voting-config. Defaults to production when no environment path is provided. Override for staging mirrors or fork testing. |
...
The `/api/voting-config` cache is a fallback — wallets, `join.sh`, and the fleet health watchdog ([`vote-infrastructure/watchdog/`](https://github.com/valargroup/vote-infrastructure/tree/main/watchdog)) read the [dynamic voting-config](https://voting.valargroup.dev/prod/dynamic-voting-config.json) directly from the gateway so it stays available if the primary `svoted` wedges.
...
|-----------|-------------|---------|
| Outbound 443 | `shielded-vote.nyc3.digitaloceanspaces.com` or the configured `SVOTE_DO_SPACES_BASE` host | `version.txt`, `svoted` + `create-val-tx` tarballs (`binaries/vote-sdk/…`), `genesis.json`, `svoted-wrapper.sh` fallback |
...
| Outbound 443 | `voting.valargroup.dev` | [`dynamic-voting-config.json`](https://voting.valargroup.dev/prod/dynamic-voting-config.json) — canonical seed-peer discovery. Defaults to production when no environment path is provided. Override via `VOTING_CONFIG_URL` for staging mirrors. |
...
| `ERROR: Could not fetch version from …/version.txt` | Outbound 443 to DO Spaces blocked | Test `DO_BASE="${SVOTE_DO_SPACES_BASE:-https://shielded-vote.nyc3.digitaloceanspaces.com}"; curl -I "${DO_BASE%/}/version.txt"`; fix egress; consider `SVOTE_LOCAL_BINARIES=1` if you already have pinned binaries on `$PATH`. |
