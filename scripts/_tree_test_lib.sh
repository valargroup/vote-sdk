...
tree_build_real_proof_load_cmd() {
  ...
  if [[ "${REAL_PROOF_INCLUDE_VC_FLOW:-0}" == "1" ]]; then
    vc_cmd="${tx_env}"'SVOTE_API_URL="'"${REAL_PROOF_API_URL}"'" HELPER_SERVER_URL="'"${REAL_PROOF_HELPER_URL}"'" SVOTE_E2E_VOTE_WINDOW_SECS="'"${REAL_PROOF_VOTE_WINDOW_SECS}"'" cargo test --release --manifest-path e2e-tests/Cargo.toml --test voting_flow_zcash_voting -- --nocapture --ignored --test-threads=1 && '"${tx_env}"'SVOTE_API_URL="'"${REAL_PROOF_API_URL}"'" HELPER_SERVER_URL="'"${REAL_PROOF_HELPER_URL}"'" cargo test --release --manifest-path e2e-tests/Cargo.toml --test atomic_delegate_cast -- --nocapture --ignored --test-threads=1'
    cat <<EOF
${sync_cmd} && ${vc_cmd}
EOF
    return
